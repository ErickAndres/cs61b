...
public class SplayTree implements Dictionary {
  ...
  private void rotateRight(BinaryTreeNode node) { 
    if (node == null || node.parent == null || node.parent.leftChild != node) { //so third check is making sure node is the left child
  ...
  private void zigZig(BinaryTreeNode node) {
    ...
    if (node == node.parent.leftChild) {
      // do we need a pointer to grandparent
  ...
  private void splayNode(BinaryTreeNode node) {
    ...
    while (node.parent != null) {
      if (node.parent.parent == null) {
        zig(node);
      } else if (node.parent == node.parent.parent.leftChild && node == node.parent.leftChild) {
        zigZig(node);
      } else if (node.parent == node.parent.parent.rightChild && node == node.parent.rightChild) {
        zigZig(node);
      } else {
        zigZag(node);    
      }
