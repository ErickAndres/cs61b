...
class SibTreeNode extends TreeNode {
  ...
  public TreeNode parent() throws InvalidNodeException {
    ...
    //Handling maybe case where tree is empty
    //SibTreeNode currNode = parent;
    /*if (this.isEmpty()) {
      throw new InvalidNodeException();
    } */
    if (!this.isValidNode()) {
      throw new InvalidNodeException();
    } else if (this.myTree.root() == this) {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    ...
    if (c <= 1) {
      SibTreeNode temp = this.firstChild; 
      ...
      SibTreeNode prev = (SibTreeNode) child(c-1);
      SibTreeNode curr = (SibTreeNode) child(c);
      prev.nextSibling = newChild;
      newChild.parent = this;
      newChild.nextSibling = curr;
      myTree.size++;
    }
  }
  ...
  public void removeLeaf() throws InvalidNodeException {
    ...
    SibTreeNode temp = (SibTreeNode) this.firstChild;
    if (temp == null) {
      if (myTree.root == this) {
        this.valid = false;
        myTree.root = null;
      }  else {
        SibTreeNode kid = (SibTreeNode) parent.firstChild;
      }
    }
  } 
}
