...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    DisjointSets maze = new DisjointSets(vert*horiz); 
    Wall [] walls = new Wall[horiz * (vert - 1) + vert * (horiz - 1)];  //constructing empty walls array with given size
    int index = 0;
    // adding vertical walls
    for (i = 0; i < horiz - 1; i++) {
      for (j = 0; j < vert; j++) {
        walls[index] = new Wall(i, j, Wall.v);
        index++;
      }
    }
    ...
    for (i = 0; i < horiz; i++) {
      for (j = 0; j < vert - 1; j++) {
        walls[index] = new Wall(i, j, Wall.h);
        index++;
      }
    }
    // randomize walls order
    for (int w = walls.length; w > 1; w--) {
      int random = randInt(w);
      Wall targetSwap = walls[random];
      walls[random] = walls[w - 1];
      walls[w - 1] = targetSwap;
    }
    // visiting wall in random order and removing chosen ones
    for (index = 0; index < walls.length; index++) {
      Wall curr = walls[index];
      int otherCellx, otherCelly;
      if (curr.getType().equals(Wall.v)) {
        otherCellx = curr.getX() + 1;
        otherCelly = curr.getY();
      } else {
        otherCellx = curr.getX();
        otherCelly = curr.getY() + 1;
      }
      int set1 = maze.find((curr.getX() * vert) + curr.getY());
      int set2 = maze.find((otherCellx * vert) + otherCelly);
      if (set1 != set2) {
        if (curr.getType().equals(Wall.v)) {
          vWalls[curr.getX()][curr.getY()] = false;
        } else {
          hWalls[curr.getX()][curr.getY()] = false;
        }
