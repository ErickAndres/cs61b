...
public class ListSorts {
  ...
  public static LinkedQueue makeQueueOfQueues(LinkedQueue q) {
    ...
    LinkedQueue allQ = new LinkedQueue(); //queue with all the different queues
    LinkedQueue oneQ;   //single queue inside big queue(allQ)
    try {
      while (q.size() > 0) {  //the queue q is still not empty
        oneQ = new LinkedQueue();  
        oneQ.enqueue(q.dequeue());  //adding an item of the queue q by dequing front item and enqueing it to oneQ
        allQ.enqueue(oneQ);  //enqueueing all oneQs to allQ
      } 
  ...
  public static LinkedQueue mergeSortedQueues(LinkedQueue q1, LinkedQueue q2) {
    // Replace the following line with your solution.
    LinkedQueue mergeQ = new LinkedQueue(); // final merged queue 
    Comparable itemQ1;
    Comparable itemQ2;
    if (q1.isEmpty() && q2.isEmpty()) {  //checking if both are empty
      return mergeQ;
    } else {
      try {
        while (q1.size() > 0 && q2.size() > 0) {
          ...
          if (itemQ1.compareTo(itemQ2) > 0) {   //if the object in the front of q1 > q2, just enqueu q2's first object at the front by dequeuing it
            mergeQ.enqueue(q2.dequeue());
          } else if (itemQ1.compareTo(itemQ2) < 0) {
            mergeQ.enqueue(q1.dequeue());    //opposite happens if q1 < q2
          } else {
            mergeQ.enqueue(q1.dequeue());
            mergeQ.enqueue(q2.dequeue());
          }
        }
        if (q1.isEmpty()) {                //if by any chance q1 is Empty, just add the contents of q2 remaining to merge@
          mergeQ.append(q2);
        } else {
          mergeQ.append(q1);               //opposite if q2 is Empty
        }
      } catch (QueueEmptyException e) {
        System.out.println(e);
      } 
 ...
 // work on lists of size zero and one.  (Our autograder will.)
  ...
  public static void main(String [] args) {
    ...
    //Test for zero size added test
    LinkedQueue z = new LinkedQueue();
    System.out.println(z.toString());
    quickSort(z);
    System.out.println(z.toString());

    //Test for size one added test
    q = makeRandom(1);
    System.out.println(q.toString());  
    quickSort(q);
    System.out.println(q.toString());
