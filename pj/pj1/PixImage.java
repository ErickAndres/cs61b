...
public class PixImage {
                ...
                for (int i = x - 1; i <= x + 1; i++) {
                	for (int j = y - 1; j <= y + 1; j++) {
                		if (i >= 0 && j >= 0 && i < width && j < height) {
                		    rgv_red +=  getRed(i, j);
                	        rgv_green += getGreen(i, j);
                	        rgv_blue += getBlue(i, j);
                	        num_squares++;
                		}
                	}
                }
                copy.setPixel(x, y, (short) (rgv_red / num_squares), (short) (rgv_green / num_squares), (short) (rgv_blue / num_squares));
      	        }
  ...
  public short[] getValue(int x,int y){
	  ...
	  for(int i = -1; i<=1; i++){
		  for(int j = -1; j<=1; j++){
			  xgrad = xsobel[i+1][j+1]; 
			  ygrad = ysobel[i+1][j+1];
			  if (x+i >= 0 && y+j >= 0 && x+i < width && y+j < height){
				  a = x+i; b = y+j;
			  } else{
				  a = x + i < 0 ? 0 : x + i >= width ? width - 1 : x + i;
				  b = y + j < 0 ? 0 : y + j >= height ? height - 1 : y + j;
