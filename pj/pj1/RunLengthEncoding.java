...
public class RunLengthEncoding implements Iterable {
  ...
  public RunLengthEncoding(int width, int height) {
    ...
    rle = new DList2();
    int[] run_len = {len, 0, 0, 0};
    rle.insertEnd(run_len);
    // Your solution here.
  }
  ...
  public RunLengthEncoding(int width, int height, int[] red, int[] green,
    ...
    for (int i = 0; i < runLengths.length; i++) {
      int[] run_len = {runLengths[i], red[i], green[i], blue[i]};
      rle.insertEnd(run_len); 
    }
    // Your solution here.
  }
  ...
  public RunIterator iterator() {
    // Replace the following line with your solution.
    RunIterator iterator = new RunIterator(rle.head.next);
    return iterator;
  ...
  public PixImage toPixImage() {
    ...
    for (int i = 0; i < width; i++) {
      for (int j = 0; j < height; j++) {
        if (counter == iter[0] && iterator.hasNext()) {   //counter == iter[0] 
            counter = 0;
            iter = iterator.next();
        }
        image.setPixel(j, i, (short) iter[1], (short) iter[2], (short) iter[3]);
        counter++;
      }
    }
    return image;
  ...
  public RunLengthEncoding(PixImage image) {
    ...
    if (width > 0 && height > 0) {
      rle = new DList2();
      rle.insertEnd(new int[] { 0, image.getRed(0, 0), image.getGreen(0, 0), image.getBlue(0, 0)});
      int[] look = rle.head.next.item;
      for (int i = 0; i < width; i++) {
        for (int j = 0; j < height; j++) {
          if (image.getRed(i, j) == look[1] && image.getGreen(i, j) == look[2] && image.getBlue(i, j) == look[3]) {
            look[0] += 1;
          } else {
            look = updated(image, i, j);
          }
        }
      }
    } 
   ...
   public int[] updated(PixImage image, int i, int j) {
    int red = image.getRed(i, j); 
    int green = image.getGreen(i, j);
    int blue = image.getBlue(i,j);
    int[] update = { 1, red ,green ,blue };
    rle.insertEnd(update);
    return update;
  } 
   ...
   */
  public void check() {
    DListNode2 head = rle.head.next;
    DListNode2 other = head.next;
    int total = 0;
    while (head.item[0] != Integer.MAX_VALUE) {
      total += head.item[0];
      if (other.item[1] == head.item[1] && other.item[2] == head.item[2] && other.item[3] == head.item[3]) {
